...
function buildPageTitle(type: SuggestionType) {
  ...
  if (type === 'PURCHASE') return 'شراء مباشر';
  return 'طلب آخر';
}
...
export default function SuggestionsPage() {
  ...
  async function fetchRows() {
    setLoading(true);
    try {
      const query = canManage ? (requestedType ? `?type=${activeType}` : '') : isCreateMode ? `?category=${activeType}` : '';
  ...
  useEffect(() => {
    setAdminNotes(selected?.adminNotesText || '');
  }, [selected]);
  ...
  function ServiceButtons() {
    const items: Array<{ type: SuggestionType; label: string }> = [
      { type: 'MAINTENANCE', label: 'طلب صيانة' },
      { type: 'CLEANING', label: 'طلب نظافة' },
      { type: 'PURCHASE', label: 'شراء مباشر' },
      { type: 'OTHER', label: 'طلب آخر' },
    ];
    ...
    return (
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {items.map((item) => (
          <Button
            key={item.type}
            variant={activeType === item.type && isCreateMode ? 'primary' : 'ghost'}
            className="w-full"
            onClick={() => router.push(`/suggestions?type=${item.type}&new=1`)}
          >
            {item.label}
          </Button>
        ))}
  ...
  function renderAttachmentSummary(row: SuggestionRow) {
    if (!row.attachments?.length) return 'لا توجد مرفقات';
    return row.attachments.map((file) => file.filename || 'مرفق بدون اسم').join('، ');
  }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <div className="font-mono text-sm font-bold text-[#016564]">{row.code || row.id}</div>
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto">
            ...
            {row.linkedDraftId && canManage ? (
              <Button variant="ghost" className="w-full sm:w-auto" onClick={() => router.push('/email-drafts')}>
                فتح المراسلات
              </Button>
            ) : null}
          </div>
    ...
    <div className="space-y-4 sm:space-y-5">
      <section className="rounded-[24px] border border-[#d6d7d4] bg-white px-4 py-4 shadow-sm sm:rounded-[28px] sm:px-5 sm:py-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          {!canManage ? (
            <Button className="w-full sm:w-auto" onClick={() => router.push('/suggestions?new=1&type=MAINTENANCE')}>
              طلب جديد
            </Button>
          ) : null}
        </div>
      ...
      {canManage ? (
        <section className="grid gap-6 xl:grid-cols-2 xl:items-start">
          ...
          <div>
            <div className="mb-3 text-lg font-bold text-[#016564]">طلبات تمت معالجتها</div>
            <div className="space-y-3">
              {loading ? [1, 2].map((i) => <Skeleton key={i} className="h-32 w-full rounded-[24px]" />) : processedRows.length ? processedRows.map(renderRow) : <Card className="rounded-[24px] border border-[#d6d7d4] p-8 text-center text-sm text-[#61706f] shadow-sm">لا توجد طلبات معالجة</Card>}
            </div>
          </div>
        </section>
      ) : (
      ...
      <Modal isOpen={isCreateMode} onClose={closeCreateMode} title={buildPageTitle(activeType)} size="2xl" bodyClassName="overflow-visible">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="space-y-2"><label className="block text-sm font-semibold text-slate-700">المرفقات (اختياري)</label><input type="file" accept="image/*,.pdf,.doc,.docx,.xlsx,.xls" multiple onChange={(e) => setAttachments(Array.from(e.target.files || []))} className="block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700" />{attachments.length > 0 ? <div className="rounded-2xl border border-[#e7ebea] bg-[#f8fbfb] px-4 py-3 text-sm text-[#304342]">الملفات المختارة: {attachments.map((file) => file.name).join(' ، ')}</div> : null}</div>
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end"><Button type="button" variant="ghost" onClick={closeCreateMode} className="w-full sm:w-auto">إلغاء</Button><Button type="submit" disabled={submitting} className="w-full sm:w-auto">{submitting ? "جارٍ الإرسال..." : "إرسال الطلب"}</Button></div>
        </form>
      </Modal>

      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title={selected ? `تفاصيل الطلب ${selected.code || ''}` : 'تفاصيل الطلب'} size="full">
        {selected ? (
          <div className="space-y-5">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-[22px] border border-[#e7ebea] bg-white p-4">
                <div className="mb-3 text-sm font-extrabold text-[#016564]">بيانات الطلب</div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">الرمز</div><div className="mt-1 text-sm leading-7 text-[#304342]">{selected.code || selected.id}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">النوع</div><div className="mt-1 text-sm leading-7 text-[#304342]">{typeMeta(resolveType(selected)).label}</div></div>
                  ...
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">التاريخ</div><div className="mt-1 text-sm leading-7 text-[#304342]">{formatDateTime(selected.createdAt)}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3 sm:col-span-2"><div className="text-xs font-bold text-[#016564]">العنوان</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.title}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3 sm:col-span-2"><div className="text-xs font-bold text-[#016564]">الوصف</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.description || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">الموقع</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.location || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">العنصر المطلوب</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.itemName || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3 sm:col-span-2"><div className="text-xs font-bold text-[#016564]">مصدر الحاجة</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.requestSource || '—'}</div></div>
                </div>
              </div>

              <div className="rounded-[22px] border border-[#e7ebea] bg-white p-4">
                <div className="mb-3 text-sm font-extrabold text-[#016564]">بيانات مقدم الطلب والمرفقات</div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3 sm:col-span-2"><div className="text-xs font-bold text-[#016564]">الاسم</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.requester?.fullName || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">الإدارة</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{selected.requester?.department || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">البريد الإلكتروني</div><div className="mt-1 break-all text-sm leading-7 text-[#304342]">{selected.requester?.email || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">التحويلة</div><div className="mt-1 text-sm leading-7 text-[#304342]">{selected.requester?.extension || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3"><div className="text-xs font-bold text-[#016564]">الجوال</div><div className="mt-1 text-sm leading-7 text-[#304342]">{selected.requester?.mobile || '—'}</div></div>
                  <div className="rounded-[18px] border border-[#e7ebea] bg-[#fcfdfd] px-4 py-3 sm:col-span-2"><div className="text-xs font-bold text-[#016564]">المرفقات</div><div className="mt-1 break-words text-sm leading-7 text-[#304342]">{renderAttachmentSummary(selected)}</div></div>
                </div>
              </div>
            </div>

            {canManage && (selected.status === 'PENDING' || selected.status === 'UNDER_REVIEW') ? (
              <div className="space-y-3 rounded-[22px] border border-[#e7ebea] bg-[#f8fbfb] p-4">
                <div className="text-sm font-extrabold text-[#016564]">قرار المدير</div>
                <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={5} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm leading-7 text-slate-800 outline-none transition focus:border-[#016564] focus:ring-4 focus:ring-[#016564]/10" placeholder="اكتب ملاحظة القرار أو التوجيه" />
                <div className="flex flex-col gap-2 sm:flex-row sm:justify-end"><Button variant="danger" className="w-full sm:w-auto" disabled={processing} onClick={() => handleDecision('reject')}>{processing ? "جارٍ المعالجة..." : "رفض الطلب"}</Button><Button className="w-full sm:w-auto" disabled={processing} onClick={() => handleDecision('approve')}>{processing ? "جارٍ المعالجة..." : "اعتماد الطلب"}</Button></div>
              </div>
            ) : null}

            <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
              {selected.linkedDraftId && canManage ? <Button className="w-full sm:w-auto" onClick={() => router.push('/email-drafts')}>فتح المراسلات الخارجية</Button> : null}
