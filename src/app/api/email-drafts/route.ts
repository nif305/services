  ...
  try {
    ...
    const data = drafts.map((draft) => {
      ...
      return {
        ...
        itemName: summarizeServiceItems(serviceItems) || String(justification.itemName || admin.itemName || linkedSuggestion?.title || '—'),
        description: linkedSuggestion?.description || stripHtmlToText(draft.body),
        attachments: attachments.map((item, index) => ({
          name: String(item.filename || item.contentType || friendlyAttachmentName(item, index)),
        })),
      };
    });
