...
function Icon({ name, className = 'h-5 w-5' }: { name: string; className?: string }) {
  ...
  switch (name) {
    case 'dashboard': return <svg viewBox="0 0 24 24" className={className}><path {...common} d="M4 13h7V4H4v9Z" /><path {...common} d="M13 20h7v-5h-7v5Z" /><path {...common} d="M13 11h7V4h-7v7Z" /><path {...common} d="M4 20h7v-5H4v5Z" /></svg>;
    ...
    case 'approvals': return <svg viewBox="0 0 24 24" className={className}><path {...common} d="M9 12l2 2 4-5" /><rect {...common} x="4" y="4" width="16" height="16" rx="3" /></svg>;
...
function buildGroups(system: SystemKey, role: Role): NavGroup[] {
  if (system === 'materials') {
    const ops: NavItem[] = [
      { href: '/materials/dashboard', label: 'لوحة معلومات المواد', icon: 'dashboard', roles: ['manager', 'warehouse', 'user'] },
      { href: '/materials/requests', label: role === 'user' ? 'طلب مواد من المخزون' : 'طلبات المواد', icon: 'requests', roles: ['manager', 'warehouse', 'user'] },
      { href: '/materials/inventory', label: 'المخزون', icon: 'inventory', roles: ['manager', 'warehouse'] },
      { href: '/materials/returns', label: role === 'user' ? 'طلبات الإرجاع' : 'المرتجعات', icon: 'returns', roles: ['manager', 'warehouse', 'user'] },
      { href: '/materials/custody', label: 'العهد', icon: 'custody', roles: ['manager', 'warehouse', 'user'] },
    ];
    return [
      { title: 'نظام المواد التدريبية', items: ops.filter(i => !i.roles || i.roles.includes(role)) },
      { title: 'المراسلات', items: [{ href: '/materials/messages', label: 'المراسلات الداخلية', icon: 'messages', roles: ['manager','warehouse','user'] }] },
      ...(role === 'manager' ? [{ title: 'الإدارة العامة', items: [{ href: '/materials/users', label: 'المستخدمون', icon: 'users', roles: ['manager'] }] }] : []),
    ];
  }

  const requestItems: NavItem[] = [
    { href: '/services/dashboard', label: 'لوحة معلومات الخدمات', icon: 'dashboard', roles: ['manager','user'] },
    { href: '/services/requests', label: 'بوابة طلبات الخدمات', icon: 'requests', roles: ['manager','user'] },
    { href: '/services/maintenance', label: 'طلبات الصيانة', icon: 'maintenance', roles: ['manager','user'] },
    { href: '/services/cleaning', label: 'طلبات النظافة', icon: 'cleaning', roles: ['manager','user'] },
    { href: '/services/purchases', label: 'طلبات الشراء المباشر', icon: 'purchases', roles: ['manager','user'] },
    { href: '/services/other', label: 'الطلبات الأخرى', icon: 'other', roles: ['manager','user'] },
  ];
  const governance: NavItem[] = [
    { href: '/services/approvals', label: 'اعتماد طلبات الخدمات', icon: 'approvals', roles: ['manager'] },
    { href: '/services/email-drafts', label: 'المراسلات الخارجية', icon: 'email', roles: ['manager'] },
    { href: '/services/messages', label: 'المراسلات الداخلية', icon: 'messages', roles: ['manager','user'] },
  ];
  return [
    { title: 'نظام الخدمات العامة', items: requestItems.filter(i => !i.roles || i.roles.includes(role)) },
    { title: 'الاعتمادات والمراسلات', items: governance.filter(i => !i.roles || i.roles.includes(role)) },
    ...(role === 'manager' ? [{ title: 'الإدارة العامة', items: [{ href: '/services/users', label: 'المستخدمون', icon: 'users', roles: ['manager'] }] }] : []),
  ];
}
...
export function SystemShell({ system, children }: { system: SystemKey; children: React.ReactNode }) {
  ...
  const title = system === 'materials' ? 'نظام المواد التدريبية' : 'نظام الخدمات العامة';
  const subtitle = system === 'materials' ? 'لوحة تنفيذ مستقلة لطلبات المواد والمخزون والمرتجعات والعهد.' : 'لوحة تنفيذ مستقلة لطلبات الخدمات والاعتمادات والمراسلات الخارجية.';
          ...
          <div className="p-4">
            <div className="rounded-[28px] border border-[#e3e9e8] bg-[#fbfcfc] p-5">
              <div className="flex justify-center rounded-[22px] border border-[#d8dfde] bg-[#f8faf9] p-4">
                <Image src="/nauss-gold-logo.png" alt="شعار الجامعة" width={220} height={110} className="h-auto w-[210px]" />
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div className="flex flex-wrap items-center gap-3">
                  <button onClick={() => router.push('/portal')} className="flex h-14 items-center gap-2 rounded-[20px] border border-[#d8dfde] bg-white px-4 text-[#2A6364] transition hover:bg-[#f7faf9]">
                    ...
                    <span className="text-sm font-semibold">اختيار النظام</span>
                  </button>

                  <div className="flex min-w-[260px] items-center gap-3 rounded-[22px] border border-[#d8dfde] bg-white px-4 py-3">
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-[20px] font-bold text-[#223b3b]">{user?.fullName || 'مستخدم النظام'}</div>
